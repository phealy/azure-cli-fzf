...
def _fzf_get_release(version):
    ...
    try:
        ...
        raise CLIError(f'Connection error while attempting to download releases list: {error}')
...
def fzf_install(version='latest', install_dir=None):
    ...
    release_iter = (asset["browser_download_url"]
                    for asset
                    in release["assets"]
                    if _fzf_get_system().upper() in asset["name"].upper()
                    and arch.upper() in asset["name"].upper()
                    )
    ...
    try:
        ...
        raise CLIError(f'Connection error while attempting to download client: {error}')

    if _fzf_get_system() == "Windows":
        zip_file = zipfile.ZipFile(compressed_file)
        zip_file.extract(install_file, path=install_dir)
    else:
        tar_file = tarfile.open(fileobj=compressed_file)
